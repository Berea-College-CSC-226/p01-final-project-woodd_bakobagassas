...
class MyTkinterApp:
    def __init__(self, windowtext="Exploring Tkinter"):
        ...
        self.myTextLabelText = tk.StringVar()  # Makes a Tkinter string variable
        self.myTextLabel1 = None

        self.menubutton = tk.Menubutton(self.root, text="Options")
        self.menu = tk.Menu(self.menubutton, tearoff=0)
        self.menubutton["menu"] = self.menu
        #self.menubutton()

    #def menubutton(self):
        self.menu.add_command(label=self.create_button1, command=self.create_button1)
        self.menu.add_command(label=self.create_button2, command=self.create_button2)
    ...
    def create_button2(self, buttontext="Push"):
        ...
        :return: None
        """
        self.myButton2 = tk.Button(self.root, text=buttontext, command=self.button1_handler)
        # Note that when myButton1 button is pushed, self.button1handler is called
    ...
    def create_label(self, labeltext2=""):
        """
        Creates a label on the window and sets the label to labeltext
        ...
        self.myTextLabelText.set(labeltext2)        # Sets the Tkinter string variable
        self.myTextLabel = tk.Label(self.root, textvariable=self.myTextLabelText)
        self.myTextLabel.pack()                    # pack means add to window
...
def main():
    ...
    myGUI.create_button2("What is your age?")  # Calls the create button method to create a button
    myGUI.create_label1()                           # Create a label to writing text into (empty for now)
    myGUI.create_label()  # Create a label to writing text into (empty for now)
    #myGUI.menubutton()
    myGUI.root.mainloop()
    # Needed to start the event loop
